...
public class Controller
{
  ...
  @FXML private CheckBox proCheck;
  ...
  @FXML protected void initialize()
  {
    ...
    //  Main.projects.stream().forEach(projects -> {});
  }

  @FXML private void showProjectPopUp(ActionEvent event)
  {
    event.consume();
    addProject();
  }
  ...
  @FXML private void showRequirementPopUp(ActionEvent event)
  {
  ...
  @FXML private void showPro(ActionEvent event)
  {
    event.consume();
    proSearch();
  }
    ...
    EventHandler<ActionEvent> event = new EventHandler<ActionEvent>()
    {
      ...
      public void handle(ActionEvent e)
      {
        ...
        char[] alphabet = "AaBbCcDdEeFfGgHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxYyZz"
            .toCharArray();
        System.out.println(Main.projects.get(0).getProjectName().charAt(0));
        System.out.println(alphabet[6]);
        if (proCheck.isSelected())
        {
          ...
          for (int i = 0; i < Main.projects.size(); i++)
          {
            ...
            {
              if (Main.projects.get(i).getProjectName().charAt(0)
                  == alphabet[k])
              {
                ...
                System.out
                    .println(Main.projects.get(i).getProjectName().charAt(0));
 ...
 */
    ...
    proCheck.setOnAction(event);

  }

  @FXML public void addProjectToList(Project project)
  {
    ...
    projectBtn.setOnAction(new EventHandler<ActionEvent>()
    {
      @Override public void handle(ActionEvent e)
      {
        ...
        requirementListContainer.getChildren().clear();
  ...
  private void addRequirement()
  {
    ...
    userStoryContainer.getChildren()
        .addAll(requirementUserStory, inputRequirementUserStory);
    ...
    deadlineContainer.getChildren()
        .addAll(requirementDeadline, inputRequirementDeadLine);
    ...
    for (int i = 0; i < Main.members.size(); i++)
    {
      ...
      int finalI = i;
      memberCheckBox.selectedProperty()
    ...
    membersContainer.getChildren()
        .addAll(requirementMembers, memberSelectionPane);
    ...
    closeButton.setOnAction(new EventHandler<ActionEvent>()
    {
      @Override public void handle(ActionEvent e)
      {
            ...
            .getText().equals("")))
        {
          ...
          currentSelected
              .addRequirement(inputRequirementName.textProperty().getValue(),
                  inputRequirementUserStory.getText(),
                  inputRequirementDeadLine.getValue(), selectedMembers);

          addRequirementToList(currentSelected.getRequirements()
              .get(currentSelected.getRequirements().size() - 1));
        }
        else
        {
          errorMessage.setText("ERROR: invalid project name");
    ...
    layout.getChildren()
        .addAll(nameContainer, userStoryContainer, deadlineContainer,
            membersContainer, errorMessage, closeButton);
    layout.setAlignment(Pos.CENTER);
  ...
  public void addRequirementToList(Requirement requirement)
  {
    ...
    Label requirementEstimatedTime = new Label(
        requirement.getEstimatedTime() + " hrs");
    ...
    TitledPane requirementTitledPane = new TitledPane(
        requirement.getTitle() + " | " + requirement.getRequirementStatus()
            .toString() + " | " + requirement.getRequirementDeadline()
            .getDayOfMonth() + "-" + requirement.getRequirementDeadline()
            .getMonthValue() + "-" + requirement.getRequirementDeadline()
            .getYear(), null);
    ...
    requirementDetail.getChildren()
        .addAll(requirementIdLabel, requirementId, requirementUserStoryLabel,
            requirementUserStoryTextArea, requirementEstimatedTimeLabel,
            requirementEstimatedTime, requirementMembersLabel,
            requirementMembersGridPane);

    requirementTitledPane.setContent(requirementPaneContainer);
    requirementPaneContainer.getChildren()
        .addAll(detailsPane, addTaskButton, taskListPane);
  ...
  public void addTask(VBox requirementPaneContainer, Requirement requirement)
  {
    ...
    estimatedTimeContainer.getChildren()
        .addAll(estimatedTime, inputEstimatedTime);
    ...
    for (int i = 0; i < requirement.getMembers().size(); i++)
    {
      RadioButton radioButton = new RadioButton(
          requirement.getMembers().get(i).getName());
    ...
    Button closeButton = new Button("Add new task");

    closeButton.setOnAction(new EventHandler<ActionEvent>()
    {
      @Override public void handle(ActionEvent e)
      {
        if (!(inputTaskTitle.getText().isEmpty() || inputTaskTitle.getText()
            .equals("")))
        {
          window.close();
          ...
          Task task = new Task(inputTaskTitle.getText(), estimatedTime,
              requirement, inputTaskDeadLine.getValue(), selectedMember[0]);
    ...
    VBox layout = new VBox(10);
    layout.getChildren()
        .addAll(titleContainer, estimatedTimeContainer, deadlineContainer,
            membersContainer, closeButton);
    layout.setAlignment(Pos.CENTER);
  ...
  public void addTaskToRequirement(VBox requirementPaneContainer, Task task)
  {
    ...
    TitledPane taskTitledPane = new TitledPane(
        task.getTaskTitle() + " | " + task.getTaskStatus().toString() + " | "
            + task.getTaskDeadline().getDayOfMonth() + "-" + task
            .getTaskDeadline().getMonthValue() + "-" + task.getTaskDeadline()
            .getYear(), null);
    taskTitledPane.setExpanded(false);

    taskPane.getChildren()
        .addAll(modifyTask, taskId, taskIdLbl, taskEstimate, taskEstiamteLbl,
